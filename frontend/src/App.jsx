...
function Dashboard({ user, onLogout, onUserUpdated }) {
  ...
  return <main className="app-shell">
    <header className="topbar"><div className="brand"><span className="brand-mark small">S</span><div><strong>StarTV</strong><span>Painel de clientes</span></div></div><div className="topbar-actions"><span className="user-chip">{user.username}</span><button className="secondary-button" onClick={() => setAccountOpen(true)}><span className="button-icon" aria-hidden="true">◉</span> Minha conta</button><button className="secondary-button" onClick={async () => { await api.logout(); onLogout(); }}><span className="button-icon" aria-hidden="true">↪</span> Sair</button></div></header>
    <section className="dashboard-heading"><div><p className="eyebrow">VISÃO GERAL</p><h1>Seus clientes</h1><p className="muted">Organize chaves e dispositivos com precisão.</p></div><button className="primary-button" onClick={() => setModal("new")}><span className="button-icon" aria-hidden="true">＋</span> Novo cliente</button></section>
    ...
    {loading ? <div className="empty-state">Carregando clientes...</div> : clients.length === 0 ? <div className="empty-state"><strong>Nenhum cliente encontrado</strong><span>Cadastre seu primeiro cliente para começar.</span></div> : <section className="client-grid">{visibleClients.map((client) => <article className="client-card" key={client.id}><div className="client-card-heading"><div><h2>{client.name}</h2></div><button className="more-button" type="button" onClick={() => setModal(client)} aria-label={`Editar ${client.name}`}>•••</button></div><div className="expiry-line"><span aria-hidden="true">◷</span>{expiryLabel(client)}</div><div className="device-summary">{client.devices.map((device) => <div className="device-summary-row" key={device.id}><span className="mono">{device.mac}</span><span>{device.app}</span><span className="device-tag">{device.key || (device.type === "tv" ? device.brand || "TV" : "Mobile")}</span></div>)}</div><div className="card-footer"><span>{client.devices.length} dispositivo{client.devices.length === 1 ? "" : "s"}</span><div><button className="text-button" onClick={() => setModal(client)}><span className="button-icon" aria-hidden="true">✎</span> Editar</button><button className="danger-button" onClick={() => removeClient(client)}><span className="button-icon" aria-hidden="true">⌫</span> Remover</button></div></div></article>)}</section>}
