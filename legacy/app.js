...
const ALERT_SVG = '<svg width="14" height="14" viewBox="0 0 12 12" fill="none" aria-hidden="true"><path d="M6 1.5 10.5 10H1.5L6 1.5Z" stroke="currentColor" stroke-width="1.4" stroke-linejoin="round" /><path d="M6 4.1V6.8M6 8.4V8.45" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" /></svg>';
...
function renderConfirmButton() {
  ...
  const labels = editMode
    ? { idle: "Confirmar alterações", pending: "Salvando…", success: "Alterações salvas", error: "Tentar novamente" }
    : { idle: "Confirmar cadastro", pending: "Cadastrando…", success: "Cliente cadastrado", error: "Tentar novamente" };

  const status = state.submitStatus;
  el.confirmBtn.disabled = status !== "idle";
  el.confirmBtn.setAttribute("aria-busy", status === "pending" ? "true" : "false");
  el.confirmBtn.setAttribute("aria-label", labels[status]);
  el.confirmBtn.classList.remove("confirm-btn--pending", "confirm-btn--success", "confirm-btn--error");
  ...
  if (status === "error") el.confirmBtn.classList.add("confirm-btn--error");

  let icon = "";
  if (status === "pending") icon = SPINNER_SVG;
  else if (status === "success") icon = CHECK_SVG;
  else if (status === "error") icon = ALERT_SVG;
  ...
  el.confirmStatus.textContent = status === "success"
    ? labels.success
    : status === "error"
...
async function handleClientSubmit(event) {
  ...
  renderConfirmButton();
  try {
    const saved = await dataLayer.saveClients(next);
    if (!saved) throw new Error("Não foi possível salvar os clientes.");
